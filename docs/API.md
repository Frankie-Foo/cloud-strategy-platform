...
状态：描述当前代码中已经实现、可以调用的 13 个 HTTP 接口
...
这组接口全部要求 `market-data:read`。它们只返回平台规定的标准化数据，不是通用 Alpaca 代理。

### 9.1 `GET /v1/market-data/events`
