...
状态：描述当前代码中已经实现、可以调用的 17 个 HTTP 接口
...
- 共享 SIP 实时事件的游标读取和可断线续传 SSE 推送；
- SIP owner、订阅、首末数据时间、时延和缺口的可观测状态；
...
除 `/health` 和 `/ready` 外，所有接口使用 Bearer token：
...
| 方法 | 路径 | Scope | 用途 |
|---|---|---|---|
| GET | `/health` | 无 | HTTP 进程存活检查，不代表行情可用 |
| GET | `/ready` | 无 | 原始库和 SIP owner 就绪检查 |
...
| GET | `/v1/market-data/status` | `market-data:read` | 订阅、连接、首末数据、时延和降级建议 |
| GET | `/v1/market-data/events` | `market-data:read` | 游标读取共享 SIP 事件 |
| GET | `/v1/market-data/stream` | `market-data:read` | 可续传 SSE 实时推送 |
...
无需认证。它只表示 HTTP 进程还活着，保持原有响应兼容；即使 SIP owner 停止、租约为空、数据已经延迟或 Alpaca 上游不可用，它仍可能返回 `ready`。监控不能只看这个接口。
...
无需认证。它检查原始事件库、有效订阅、SIP owner 状态、90 秒内心跳，以及 owner 当前标的与有效订阅并集是否一致。
...
任一条件不满足时返回 HTTP 503，`status=not_ready`。该接口不公开标的和错误细节；详细诊断使用带 `market-data:read` token 的 `/v1/market-data/status`。
...
读取接口要求 `market-data:read`；订阅协调接口要求 `market-data:write`，该 scope 同时包含读取能力。它们只返回平台规定的标准化数据，不是通用 Alpaca 代理。实时账本保留每根分钟 bar；quote 按“每个 symbol、每个 UTC 秒最多一条”采样，防止高频报价挤压分钟 bar 和其他标的。客户端应先查询 `status`，再决定正常使用、等待、暂停策略或切换带 provenance 的备用源。
...
### 9.2 `GET /v1/market-data/status`

查询 SIP owner 心跳、连接状态、当前订阅并集，以及每个标的从何时有数据、最后一根 bar/quote、累计数量、最后游标和时延。

```http
GET /v1/market-data/status?symbols=AAPL,SMCI HTTP/1.1
...
| 字段 | 含义 |
|---|---|
| `status` | 整体 `healthy / degraded / unavailable / market_closed` |
| `fallback_recommended` | 策略是否应考虑暂停或切换备用源 |
| `owner.state` | `starting / waiting_for_subscription / connecting / connected / reconnecting / error / stopped / unavailable` |
| `owner.heartbeat_age_seconds` | owner 心跳延迟；超过 90 秒不再算 ready |
| `subscription.active_symbols` | 当前有效租约和活动策略的订阅并集 |
| `symbols[].first_event_at_utc` | 本地账本第一次有该标的数据时间 |
| `symbols[].last_bar_at_utc` | 最后一根实时分钟 bar |
| `symbols[].last_quote_at_utc` | 最后一条采样 quote |
| `symbols[].status` | `healthy / delayed / stale / unavailable / market_closed` |
| `symbols[].reason_codes` | `not_subscribed`、`sip_owner_not_ready`、`no_events_observed`、时延等机器可读原因 |

`execution_health.status=external_not_observed` 是刻意设计：当前交易执行由外部盈透模拟链路负责，本服务不能把“行情健康”伪装成“可下单或可成交”。

### 9.3 `GET /v1/market-data/events`（兼容读取）

用于旧版 AI 行情客户端按游标读取云端共享 SIP 事件日志。新客户端应优先使用 9.4 的 SSE；此接口保留给兼容客户端、审计回放和短批次读取。
...
### 9.4 `GET /v1/market-data/stream`

通过 Server-Sent Events 推送已经落入共享账本的标准化 bar 和 quote，客户端不再需要不停轮询。请求参数：

| 参数/Header | 必填 | 默认 | 规则 |
|---|---|---|---|
| `symbols` | 是 | 无 | 英文逗号分隔，最多 500 个 |
| `after` | 否 | `0` | 排他 sequence 游标 |
| `Last-Event-ID` | 否 | 无 | 断线重连游标；`after` 存在时以 `after` 为准 |
| `heartbeat_seconds` | 否 | `15` | `5..60` |

```http
GET /v1/market-data/stream?symbols=AAPL,MSFT&after=1200 HTTP/1.1
...
id: 1201
event: market-data
data: {"sequence":1201,"event":{"event_type":"bar","symbol":"AAPL","ts_utc":"2026-07-22T15:30:00Z"}}

: heartbeat
```

客户端只在业务处理成功后保存 SSE `id`；断线后通过 `Last-Event-ID` 或 `after` 继续。AI 投资端当前实现会优先连接这个接口，仅当服务明确返回 `404/405`（旧服务没有 SSE）时才退回 9.3 的游标轮询；认证失败、契约错误或行情健康失败不会静默回退。该流来自耐久账本，不暴露 Alpaca provider WebSocket、凭据或任意代理能力。反向代理必须关闭响应缓冲并允许长连接。

### 9.5 `GET /v1/market-data/bars`

返回 SIP feed 的 `1Min`、split-adjusted 历史分钟线。适配器会自动读取 Alpaca 的所有分页，再合并成一个响应。响应保留原有 `bars` 数组，并新增 `coverage`，所以旧客户端不受破坏。
...
- `status=observed / empty / gaps_detected / invalid_timestamps`；
- `row_count`、首末时间和末条数据相对查询终点的时延；
- 正常交易时段内相邻 bar 的 `missing_intervals` 和 `missing_minute_count`；
- 可能整天缺失的 `potential_missing_regular_dates`；
- `reason_codes`，例如 `upstream_empty`、`regular_session_missing`、`minute_gaps_detected`；
- `fallback_recommended`，供策略自动降级。

空响应现在不再只有 `bars=[]`。例如盘中 SMCI 上游返回空列表时，同时返回：
    ...
    "symbols": [
      {
        ...
        "potential_missing_regular_dates": ["2026-07-22"],
        "reason_codes": ["upstream_empty", "regular_session_missing"]
      }
    ]
  }
}
...
缺口判断使用 America/New_York 工作日和正常交易时段，并明确标注“交易所节假日未验证”；因此整天缺失日期是诊断提示，不冒充完整交易所日历证明。

### 9.6 `GET /v1/market-data/quotes`

参数与 bars 相同，也返回 `coverage`；quote 不按每分钟必然出现来推断缺口，只报告空响应、首末时间和时延。响应：
...
- SSE 的 `id` 与耐久事件 `sequence` 相同；断线重连必须带最后成功处理的游标；
- `/health=ready` 只能用于进程存活检查；策略启动和运行降级判断必须读取 `/ready` 与 `/v1/market-data/status`；
- 当历史 `coverage.fallback_recommended=true` 或实时 `fallback_recommended=true` 时，AI 投资端失败关闭，不会自动混入未经批准的 Yahoo/社区数据；
...
6. SSE 已解决客户端轮询，但当前仍是单机 SQLite/本地进程部署，尚未完成公网 HTTPS、多机故障切换和外部 SLA 验证；
7. 行情健康度不包含外部盈透模拟执行链路；策略必须把行情健康和交易通道健康分别检查；
8. 交易时段诊断使用纽约时区工作日近似，交易所节假日仍需正式 calendar 数据源确认；
9. 列表接口除 SIP events 外没有分页契约；
10. 暂无请求 ID、限流响应头、异步 job 和 webhook 契约。
